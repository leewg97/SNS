...
public class UserControllerTest {
    ...
    @WithMockUser
    @DisplayName("알림 목록 요청 기능")
    @Test
    void requestNotificationListSuccessful() throws Exception {
        when(userService.notificationList(any(), any())).thenReturn(Page.empty());
        mockMvc.perform(get("/api/v1/users/notify")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(status().isOk());
    }
    ...
    @WithAnonymousUser
    @DisplayName("알림 리스트 요청시 로그인 하지 않은 경우")
    @Test
    void ifYouAreNotLoggedInBeforeRequestingNotificationList() throws Exception {
        when(userService.notificationList(any(), any())).thenReturn(Page.empty());
        mockMvc.perform(get("/api/v1/users/notify")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
